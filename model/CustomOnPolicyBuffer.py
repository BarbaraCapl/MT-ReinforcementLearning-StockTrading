...
class OnPolicyBuffer():
    """ This class implements the On-Policy-Buffer, which does the following:
    ...
    """
    def __init__(self,
                 buffer_size: int, # buffer size always = length of the data set here
                 obs_shape: Tuple[int, int], # must be tuple (observations,)
                 actions_number: int,
                 lstm_obs_shape: bool=None,
        ):
        ...
        # regarding shapes: the array needs to be of length buffer_size, then it needs to be able to store arrays of shape (2,1,64) = 2 arrays of shape 1*64
        self.lstm_state_h = np.zeros((self.buffer_size,) + (2,1) + (32,), dtype=np.float32) 
        self.lstm_state_c = np.zeros((self.buffer_size,) + (2,1) + (32,), dtype=np.float32)
        self.lstm_state_actor_h = np.zeros((self.buffer_size,)+ (2,1) + (32,), dtype=np.float32)
        self.lstm_state_actor_c = np.zeros((self.buffer_size,)+ (2,1) + (32,), dtype=np.float32)
        self.lstm_state_critic_h = np.zeros((self.buffer_size,)+ (2,1) + (32,), dtype=np.float32)
        self.lstm_state_critic_c = np.zeros((self.buffer_size,)+ (2,1) + (32,), dtype=np.float32)
