...
class PPO_algorithm():
    """ This class is a custom implementation of the Proximal Policy Optimization (PPO) algorithm based on 
    and extended from the paper: https://arxiv.org/abs/1707.06347 of Schulman et al (2017).
    ...
    def _collect_experiences_to_buffer(self, total_timesteps_to_collect):
        ...
        if self.Agent.net_arch == "mlplstm_separate":
            lstm_state = None # no shared lstm state, only one separate for actor and a separate one for critic
            lstm_state_actor = self.Agent.feature_extractor_actor.create_initial_lstm_state(sequence_length=1)
            lstm_state_critic = self.Agent.feature_extractor_critic.create_initial_lstm_state(sequence_length=1)
        
        elif self.Agent.net_arch == "mlplstm_shared":
            lstm_state = self.Agent.feature_extractor.create_initial_lstm_state(sequence_length=1)
        ...
        while current_timesteps_collected < total_timesteps_to_collect:
            ...
            with torch.no_grad():
                ...
                # get value estimate, sampled action and actions log probabilities from Agent,
            ...
            if self.Agent.net_arch == "mlplstm_separate":
                lstm_state = None
                # set the next lstm states as the ones we just received above from agent
    ...
    def learn(self,
        ...
        while learning_timesteps_done < total_learning_timesteps:
            ...
            for epoch in range(1, self.num_epochs + 1):
                ...
                for batch_num, batch_idx in enumerate(range(self.batch_size, self.buffer_size+1, self.batch_size)):
                    ...
                    new_V_estimate, current_action_new_log_prob, action_distr_entropy, action_means, _, stdev = \
                        self.Agent(observations=batch_obs, #torch.as_tensor(batch_obs, dtype=torch.float),
                                   ...
                                   lstm_states_critic=first_lstm_state_critic)  # evaluation mode must be true: see documentation in Agent class
                    ...
                    # -----------------------------------------#
                    ### 2c. UPDATE Agent (WHOLE NETWORK(s) ) ###
                    # -----------------------------------------#
                    ...
                    torch.nn.utils.clip_grad_norm_(self.Agent.parameters(), self.max_gradient_normalization)
                    ...
                    if epoch == 3 and learning_timesteps_done==0 and batch_num==1:
                        self.logger.info(f"(epoch: {epoch}, Agent gradients before optimizer step:")
                        for name, param in self.Agent.named_parameters():
                            if param.requires_grad:
            ...
            if learning_timesteps_done >= total_learning_timesteps-64:
                self.logger.info("-Agent- parameters after training: ")
    ...
    def predict(self, new_obs, env_step_version, n_assets,
                ...
                predict_deterministic: bool=False):
        """
        Function of PPO agent which calls the predict function of Agent. Predicts actions given observations.
        ...
        with torch.no_grad():
            ...
            lstm_state, lstm_state_actor, lstm_state_critic = self.Agent(observations=new_obs,
        ...
        if self.Agent.net_arch == "mlplstm_shared":
            lstm_state = (lstm_state[0].detach(), lstm_state[1].detach())
            lstm_state_actor=None
            lstm_state_critic=None
        elif self.Agent.net_arch == "mlplstm_separate":
            lstm_state_actor = (lstm_state_actor[0].detach(), lstm_state_actor[1].detach())
            lstm_state_critic = (lstm_state_critic[0].detach(), lstm_state_critic[1].detach())
            lstm_state=None
        else:
